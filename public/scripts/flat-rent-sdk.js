...
export class FlatRentSdk {
     ...
     * @param {Date} parameters.checkin Check-in date
     * @param {Date} parameters.checkout Check-out date
     * @param {number} [parameters.maxprice] Max price for a night
    ...
    search(parameters) {
        return new Promise((resolve, reject) => {
            try {
                ...
                const checkInDate = parameters.checkin;
                const checkOutDate = parameters.checkout;
                if (!(checkInDate instanceof Date) || !(checkOutDate instanceof Date)) {
                    throw new Error(`Passed invalid check-in or check-out date - from "${checkInDate}" to "${checkOutDate}".`);
                }
                this._assertDatesAreCorrect(checkInDate, checkOutDate);
                if (parameters.priceLimit != null && (isNaN(parameters.maxprice) || !isFinite(parameters.maxprice))) {
                    throw new Error(`Passed invalid price limit - "${parameters.maxprice}".`);
                }
                let flats = this.database;
                if (parameters.maxprice != null) {
                    flats = flats.filter((flat) => {
                        return flat.price <= parameters.maxprice;
                    });
                }
                const dateRange = this._generateDateRange(checkInDate, checkOutDate);
                flats = flats.filter((flat) => {
